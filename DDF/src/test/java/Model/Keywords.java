...
public class Keywords extends Base_Class{
	...
	{	
		...
		else
		{
			...
			Base_Class.log.debug("Invalid Browse Name Hence open Defaul Browser as => Chrome Browser");
			driver =new ChromeDriver();
			this.action=new Actions(driver);
		}
	}
	...
	public boolean EnterText(String locator, String locatortype, String input)
	{
		...
		case "tagname":
			...
			break;
		default:
			Base_Class.log.debug("Invalid Locator Type Entered in Sheet, Kindly Check and corrent the same, It should be from ('xpath','linktext','classname','id','cssselector','name','partiallinktext','tagname',)");
			break;
		}
		Base_Class.log.debug("Entering Data/Input => " + input + " into => " + locatortype +" =>" + locator);
	}
		catch(Exception e)
		{
			Base_Class.log.debug("Unable to Locate => "+ locatortype + " => " + locator + " on page");
		}
		return response;
	}
